...
module ``08: Putting the Function into Functional Programming`` = 
    ...
    let ``04 The input to a function is a pattern (Part 3).`` () =
        ...
        (fun { Author=k } -> "Author is " + k) {Author="Plato"; Title="  "; Year = 2000}  |> should equal "Author is Plato"
    ...
    let ``19 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows =  fun z -> f "cow" (z +  ", de gozaru")  // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
    ...
    let ``21 Getting closure`` () =
        ...
        // note the number of inputs provided below.  Do you see why I can do this? 5 is the t
    ...
    *)
    ...
    let ``26 Functions have types`` () =
        ...
        b 14 -8.7 |> should be ofType<string -> char -> bool -> string> //returns the typs that are missing
    ...
    *)
    ...
    let ``30 The 'inline' keyword forces type-resolution at callsite`` () =
        ...
        a 0.2 1.0 |> should equal 1.2 // expected: a float
