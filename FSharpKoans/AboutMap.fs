   ...
   *)
    ...
    let ``03 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
              let rec mapS func listA result = 
                match listA with
                |[] -> List.rev result
                |head::tail -> mapS func tail ((func head)::result)
              mapS f xs [] // write a map which applies f to each element
